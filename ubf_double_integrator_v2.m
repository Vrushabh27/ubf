...
% Author: Vrushabh Zinage (Modified by ChatGPT)
...
m_order = 2;              % Order of the global higher-order UBF (m_order >=1)
...
N = 35;                   % Number of Forward-Euler steps
...
x_goal = [5; 4;0;0];      % Example goal state for a 2D system
...
% h3: Control input constraint (e.g., ||u|| <= sqrt(200))
...
for i = 1:num_ubfs
    ...
    for order = 2:m_order_indiv(i)
        ...
        % Compute dh/dx and dh/du of the previous UBF (h_prev)
...
x_current =  [1;1;0;0];%zeros(n,1);             % Initial state
...
for k = 2:num_steps
    k
    % Display current step (optional for debugging)
    % disp(['Step: ', num2str(k)]);
...
figure;
plot(time, (u_traj(1,:).^2 + u_traj(2,:).^2), 'LineWidth', 2);
xlabel('Time (s)', 'Interpreter', 'latex');
